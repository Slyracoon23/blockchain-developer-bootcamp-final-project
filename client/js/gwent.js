...
class DeckMaker {
	constructor() {
		...
		this.yourDeck_elem = document.getElementById("my-card-deck");
		...
		this.makeYourDeck(this.faction)
	...
	makeYourDeck(faction) {
		// clear cards
		this.clearYourDeck();

		GetGwentCards().then( result => {
			console.log(result);
			let NFTs = result.map(Number);
		

			// creating mapping of cards and index
			let cards = card_dict.map((c,i) => ({card:c, index:i}))
			...
			// create Preview
			cards.forEach( p => {
				this.makePreviewYourDeck(p.index, Number.parseInt(p.card.count), this.yourDeck_elem, this.yourDeck);
			});
		});
	...
	makePreviewYourDeck(index, num, container_elem, cards){
		let card_data = card_dict[index];
	...
	clearYourDeck(){
		while (this.yourDeck_elem.firstChild)
			this.yourDeck_elem.removeChild(this.yourDeck_elem.firstChild);
		this.yourDeck = [];
	}
